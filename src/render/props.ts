...
export function buildProps(seed: number): PropsResult {
  ...
  PROPS.stalls.forEach((s, i) => {
    ...
    if (i === 1 || i === 4) {
      // Demirci Mehmet (z1) / Zırhçı Mehmet (z3): forge-front dressing
  ...
  // ---- graveyards: headstone shapes, leaning, instanced --------------------
  const graveKinds: PropKey[] = ['graveRound', 'graveDecor', 'graveBevel', 'graveRound'];
