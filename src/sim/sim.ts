...
export class Sim {
  ...
  constructor(cfg: SimConfig) {
    this.cfg = {
      ...
      playerName: cfg.playerName ?? 'Zafer',
    };
  ...
  removePlayer(pid: number): void {
    ...
    this.removeFromParty(pid, 'gruptan ayrıldı');
  ...
  private standUp(p: Entity): void {
    ...
    if (isConsuming(p)) {
      p.eating = null;
      p.drinking = null;
      this.emit({ type: 'log', text: 'Ayağa kalkıyorsun.', color: '#999', pid: p.id });
    }
  }
  ...
  castAbility(abilityId: string, pid?: number): void {
    ...
    if (this.isStunned(p)) { this.error(p.id, 'Sersemlemiş durumdasın!'); return; }
    if (p.castingAbility) { this.error(p.id, 'Meşgulsün.'); return; }
    if (!ability.offGcd && p.gcdRemaining > 0) return; // silent, classic spams this
    if (p.cooldowns.has(ability.id)) { this.error(p.id, 'Bu yetenek henüz hazır değil.'); return; }
    if (p.resource < res.cost) {
      this.error(p.id, p.resourceType === 'rage' ? 'Yeterli öfke yok!' : p.resourceType === 'energy' ? 'Yeterli enerji yok!' : 'Yeterli mana yok!');
      return;
    }
    if (ability.requiresDodgeProc && this.time > p.overpowerUntil) {
      this.error(p.id, 'Önce hedefin savuşturmalı.');
      return;
    }
    if (ability.spendsCombo && (p.comboPoints <= 0 || p.comboTargetId !== p.targetId)) {
      this.error(p.id, 'Bu yetenek kombo puanı gerektirir.');
      return;
    }
    ...
    if (ability.requiresTarget && ability.targetType === 'friendly') {
      ...
      if (d > Math.max(ability.range, 5)) { this.error(p.id, 'Menzil dışında.'); return; }
    } else if (ability.requiresTarget) {
      target = p.targetId !== null ? this.entities.get(p.targetId) ?? null : null;
      if (!target || target.dead || !this.isHostileTo(p, target)) { this.error(p.id, 'Hedefin yok.'); return; }
      const d = dist2d(p.pos, target.pos);
      const maxRange = ability.range > 0 ? ability.range : MELEE_RANGE;
      if (d > maxRange) { this.error(p.id, 'Menzil dışında.'); return; }
      if (ability.minRange && d < ability.minRange) { this.error(p.id, 'Çok yakın!'); return; }
      const facingDiff = Math.abs(normAngle(angleTo(p.pos, target.pos) - p.facing));
      if (facingDiff > MELEE_ARC) { this.error(p.id, 'Hedefine dönük olmalısın.'); return; }
      ...
      for (const eff of res.effects) {
        if (eff.type === 'weaponStrike' && eff.requiresBehind) {
          if (!p.weapon.dagger) { this.error(p.id, 'Hançer kuşanmalısın.'); return; }
          const behindDiff = Math.abs(normAngle(angleTo(target.pos, p.pos) - target.facing));
          if (behindDiff < Math.PI / 2) { this.error(p.id, 'Hedefinin arkasında olmalısın.'); return; }
        }
        if (eff.type === 'polymorph') {
          if (target.kind !== 'mob') { this.error(p.id, 'Bu yaratık şekil değiştirilemez.'); return; }
          const fam = MOBS[target.templateId]?.family;
          if (fam === 'undead' || target.templateId === 'gorrak') { this.error(p.id, 'Bu yaratık şekil değiştirilemez.'); return; }
        }
        if (eff.type === 'judgement' && !p.auras.some((a) => a.kind === 'imbue' && a.value2 !== undefined)) {
          this.error(p.id, 'Etkin bir Mührün yok.');
          return;
        }
      }
    }
  ...
  private applyAbility(p: Entity, meta: PlayerMeta, res: ResolvedAbility): void {
    ...
    if (ability.requiresTarget && ability.targetType === 'friendly') {
      const cur = p.targetId !== null ? this.entities.get(p.targetId) ?? null : null;
      target = cur && !cur.dead && this.isFriendlyTo(p, cur) ? cur : p;
      if (dist2d(p.pos, target.pos) > Math.max(ability.range, 5) + 2) { this.error(p.id, 'Menzil dışında.'); return; }
    } else if (ability.requiresTarget) {
      target = p.targetId !== null ? this.entities.get(p.targetId) ?? null : null;
      if (!target || target.dead) { this.error(p.id, 'Hedefin yok.'); return; }
      const d = dist2d(p.pos, target.pos);
      const maxRange = ability.range > 0 ? ability.range : MELEE_RANGE;
      if (d > maxRange + 2) { this.error(p.id, 'Menzil dışında.'); return; }
    }
    if (p.resource < res.cost) { this.error(p.id, 'Yeterli kaynak yok!'); return; }
    ...
    for (const eff of res.effects) {
      switch (eff.type) {
        ...
        case 'judgement': {
          ...
          if (sealIdx < 0) { this.error(p.id, 'Etkin bir Mührün yok.'); break; }
        ...
        case 'lifeTap': {
          if (p.hp <= eff.hp) { this.error(p.id, 'Yeterli can yok.'); break; }
          p.hp -= eff.hp;
  ...
  startAutoAttack(pid?: number): void {
    ...
    if (!t || t.dead || !this.isHostileTo(p, t)) { this.error(p.id, 'Geçersiz saldırı hedefi.'); return; }
    if (p.sitting) this.standUp(p);
    p.autoAttack = true;
  }
  ...
  private updateBossMechanics(mob: Entity): void {
    ...
    if (tmpl.enrage && !mob.enraged && hpFrac <= tmpl.enrage.belowHpPct) {
      ...
      this.emit({ type: 'log', text: `${mob.name} öfkeye kapılıyor!`, color: '#ff6666', entityId: mob.id });
  ...
  private spawnBossAdds(boss: Entity, mobId: string, count: number): void {
    ...
    if (!template) return;
    this.emit({ type: 'log', text: `${boss.name} yardım çağırıyor!`, color: '#ff6666', entityId: boss.id });
  ...
  addItem(itemId: string, count: number, pid?: number): void {
    ...
    this.emit({ type: 'loot', text: `Aldın: ${def?.name ?? itemId}${count > 1 ? ' x' + count : ''}.`, pid: meta.entityId });
  ...
  equipItem(itemId: string, pid?: number): void {
    ...
    if (def.requiredClass && !def.requiredClass.includes(meta.cls)) {
      this.error(meta.entityId, 'Bunu kuşanamazsın.');
      return;
    }
    const slot = def.slot;
    ...
    recalcPlayerStats(p, meta.cls, meta.equipment);
    this.emit({ type: 'log', text: `${def.name} kuşanıldı.`, color: '#8f8', pid: meta.entityId });
  }
  ...
  buyItem(npcId: number, itemId: string, pid?: number): void {
    ...
    if (dist2d(p.pos, npc.pos) > INTERACT_RANGE + 2) { this.error(meta.entityId, 'Çok uzakta.'); return; }
    if (meta.copper < def.buyValue) { this.error(meta.entityId, 'Yeterli para yok.'); return; }
  ...
  sellItem(itemId: string, pid?: number): void {
    ...
    if (!nearVendor) { this.error(meta.entityId, 'Yakında tüccar yok.'); return; }
    if (def.kind === 'quest') { this.error(meta.entityId, 'Görev eşyalarını satamazsın.'); return; }
    this.removeItem(itemId, 1, meta.entityId);
    meta.copper += def.sellValue;
    this.emit({ type: 'vendor', action: 'sell', itemId, pid: meta.entityId });
    this.emit({ type: 'loot', text: `${def.name} ${formatMoney(def.sellValue)} karşılığında satıldı.`, pid: meta.entityId });
  }
  ...
  lootCorpse(mobId: number, pid?: number): void {
    ...
    if (dist2d(p.pos, mob.pos) > INTERACT_RANGE) { this.error(meta.entityId, 'Çok uzakta.'); return; }
    if (mob.loot.copper > 0) {
      meta.copper += mob.loot.copper;
      meta.counters.lootCopper += mob.loot.copper;
      this.emit({ type: 'loot', text: `${formatMoney(mob.loot.copper)} yağmaladın.`, pid: meta.entityId });
    }
  ...
  pickUpObject(objId: number, pid?: number): void {
    ...
    if (dist2d(p.pos, obj.pos) > INTERACT_RANGE) { this.error(meta.entityId, 'Çok uzakta.'); return; }
    const def = ITEMS[obj.objectItemId];
    if (def?.questId) {
      const qp = meta.questLog.get(def.questId);
      if (!qp || qp.state !== 'active') { this.error(meta.entityId, 'Sıkıca çivilenmiş.'); return; }
      ...
      if (objIdx >= 0 && this.countItem(obj.objectItemId, meta.entityId) >= quest.objectives[objIdx].count) {
        this.error(meta.entityId, 'Bunlardan yeterince var.');
        return;
      }
    }
  ...
  acceptQuest(questId: string, pid?: number): void {
    ...
    this.emit({ type: 'log', text: `Görev kabul edildi: ${quest.name}`, color: '#ff0', pid: meta.entityId });
    this.onInventoryChangedForQuests(meta);
  }
  ...
  turnInQuest(questId: string, pid?: number): void {
    ...
    if (!npc || dist2d(p.pos, npc.pos) > INTERACT_RANGE + 2) { this.error(meta.entityId, 'Çok uzakta.'); return; }
    ...
    if (quest.copperReward > 0) {
      ...
      this.emit({ type: 'loot', text: `${formatMoney(quest.copperReward)} aldın.`, pid: meta.entityId });
    }
    ...
    this.emit({ type: 'log', text: `Görev tamamlandı: ${quest.name}`, color: '#ff0', pid: meta.entityId });
  }
  ...
  private checkQuestReady(qp: QuestProgress, meta: PlayerMeta): void {
    ...
    if (ready && qp.state === 'active') {
      ...
      this.emit({ type: 'log', text: `${quest.name} (Tamamlandı)`, color: '#ff0', pid: meta.entityId });
  ...
  chat(text: string, pid?: number): void {
    ...
    if (clean.startsWith('/p ') || clean.startsWith('/party ')) {
      ...
      if (!party) { this.error(r.meta.entityId, 'Bir grupta değilsin.'); return; }
      for (const mPid of party.members) {
  ...
  partyInvite(targetPid: number, pid?: number): void {
    ...
    if (myParty && myParty.leader !== r.meta.entityId) { this.error(r.meta.entityId, 'Yalnızca grup lideri davet edebilir.'); return; }
    if (myParty && myParty.members.length >= PARTY_MAX) { this.error(r.meta.entityId, 'Grubun dolu.'); return; }
    ...
    this.emit({ type: 'log', text: `${target.name} oyuncusunu grubuna davet ettin.`, color: '#aaf', pid: r.meta.entityId });
  }
  ...
  partyAccept(pid?: number): void {
    ...
    if (!invite || invite.expires < this.time) { this.error(r.meta.entityId, 'Davetin süresi doldu.'); return; }
    ...
    if (party.members.length >= PARTY_MAX) { this.error(r.meta.entityId, 'O grup dolu.'); return; }
    party.members.push(r.meta.entityId);
    this.partyByPid.set(r.meta.entityId, party.id);
    for (const mPid of party.members) {
      this.emit({ type: 'log', text: `${r.meta.name} gruba katılıyor.`, color: '#aaf', pid: mPid });
    }
  }
  ...
  partyDecline(pid?: number): void {
    ...
    this.partyInvites.delete(r.meta.entityId);
    if (invite) {
      this.emit({ type: 'log', text: `${r.meta.name} davetini reddediyor.`, color: '#aaf', pid: invite.fromPid });
    }
  }
  ...
  partyLeave(pid?: number): void {
    const r = this.resolve(pid);
    if (!r) return;
    this.removeFromParty(r.meta.entityId, 'gruptan ayrılıyor');
  }
  ...
  partyKick(targetPid: number, pid?: number): void {
    ...
    if (!r) return;
    const party = this.partyOf(r.meta.entityId);
    if (!party || party.leader !== r.meta.entityId) { this.error(r.meta.entityId, 'Grup lideri değilsin.'); return; }
    if (!party.members.includes(targetPid) || targetPid === r.meta.entityId) return;
    this.removeFromParty(targetPid, 'gruptan çıkarıldı');
  }
  ...
  private removeFromParty(pid: number, verb: string): void {
    ...
    for (const mPid of [...party.members, pid]) {
      this.emit({ type: 'log', text: `${meta?.name ?? 'Birisi'} ${verb}.`, color: '#aaf', pid: mPid });
    }
    if (party.members.length <= 1) {
      for (const mPid of party.members) {
        this.partyByPid.delete(mPid);
        this.emit({ type: 'log', text: 'Grubun dağıldı.', color: '#aaf', pid: mPid });
      }
      ...
      for (const mPid of party.members) {
        this.emit({ type: 'log', text: `${newLeader?.name ?? 'Birisi'} artık grup lideri.`, color: '#aaf', pid: mPid });
      }
    }
  }
  ...
  duelRequest(targetPid: number, pid?: number): void {
    ...
    if (targetPid === r.meta.entityId) return;
    if (this.duels.has(r.meta.entityId) || this.duels.has(targetPid)) { this.error(r.meta.entityId, 'Zaten süren bir düello var.'); return; }
    if (dist2d(r.e.pos, targetE.pos) > 30) { this.error(r.meta.entityId, 'Hedef çok uzakta.'); return; }
    ...
    this.emit({ type: 'log', text: `${target.name} oyuncusuna düello için meydan okudun.`, color: '#fa6', pid: r.meta.entityId });
  }
  ...
  duelAccept(pid?: number): void {
    ...
    if (!invite || invite.expires < this.time) { this.error(r.meta.entityId, 'Meydan okumanın süresi doldu.'); return; }
  ...
  duelDecline(pid?: number): void {
    ...
    this.duelInvites.delete(r.meta.entityId);
    if (invite) {
      this.emit({ type: 'log', text: `${r.meta.name} meydan okumanı reddediyor.`, color: '#fa6', pid: invite.fromPid });
    }
  }
    ...
    for (const duel of this.duels.values()) {
      ...
      if (duel.state === 'countdown') {
        ...
        if (duel.timer <= 0) {
          duel.state = 'active';
          for (const dPid of [duel.a, duel.b]) {
            this.emit({ type: 'log', text: 'Düello başladı!', color: '#fa6', pid: dPid });
          }
        }
        continue;
      }
  ...
  private endDuel(duel: DuelState, winnerPid: number | null): void {
    ...
    if (winnerPid !== null && aMeta && bMeta) {
      ...
      for (const dPid of [duel.a, duel.b]) {
        this.emit({ type: 'log', text: 'Düello sona erdi.', color: '#fa6', pid: dPid });
      }
    }
  }
  ...
  tradeRequest(targetPid: number, pid?: number): void {
    ...
    if (!r || !target || !targetE) return;
    if (targetPid === r.meta.entityId) return;
    if (this.trades.has(r.meta.entityId) || this.trades.has(targetPid)) { this.error(r.meta.entityId, 'Zaten süren bir takas var.'); return; }
    if (dist2d(r.e.pos, targetE.pos) > TRADE_RANGE) { this.error(r.meta.entityId, 'Hedef takas için çok uzakta.'); return; }
    ...
    this.emit({ type: 'log', text: `${target.name} ile takas isteğinde bulundun.`, color: '#8df', pid: r.meta.entityId });
  }
  ...
  tradeAccept(pid?: number): void {
    ...
    if (!invite || invite.expires < this.time) { this.error(r.meta.entityId, 'Takas isteğinin süresi doldu.'); return; }
    ...
    for (const tPid of [session.a, session.b]) {
      this.emit({ type: 'log', text: 'Takas penceresi açıldı.', color: '#8df', pid: tPid });
    }
  }
  ...
  tradeConfirm(pid?: number): void {
    ...
    if (!valid) {
      for (const tPid of [session.a, session.b]) this.error(tPid, 'Takas başarısız: eşyalar ya da para artık mevcut değil.');
    ...
    for (const tPid of [session.a, session.b]) {
      this.emit({ type: 'log', text: 'Takas tamamlandı.', color: '#8df', pid: tPid });
    }
    this.closeTrade(session);
  }
  ...
  tradeCancel(pid?: number): void {
    ...
    if (!session) return;
    for (const tPid of [session.a, session.b]) {
      this.emit({ type: 'log', text: 'Takas iptal edildi.', color: '#8df', pid: tPid });
    }
    this.closeTrade(session);
  }
  ...
  enterDungeon(dungeonId: string, pid?: number): void {
    ...
    if (!party || party.members.length < dungeon.suggestedPlayers) {
      this.emit({ type: 'log', text: `${dungeon.name}, ${dungeon.suggestedPlayers} kişilik tam bir grup içindir. Dikkatli ilerle.`, color: '#f96', pid: r.meta.entityId });
    }
