...
function newEncounter(now: number): Encounter {
  return {
    label: 'Savaş', startedAt: now, duration: 0, tallies: new Map(),
...
export class MeterData {
  ...
  constructor(now: number) {
    this.allTime = { ...newEncounter(now), label: 'Tümü (oturum)' };
  }
...
const TAB_LABEL: Record<Tab, string> = { dmg: 'Hasar', heal: 'İyileştirme', threat: 'Tehdit' };
...
export class Meters {
  ...
  private viewedEncounter(): { enc: Encounter | null; viewName: string } {
    ...
    if (this.viewIdx === h.length + 1 || (this.viewIdx > 0 && h.length === 0)) {
      return { enc: this.data.allTime, viewName: 'Tümü (oturum)' };
    }
    if (this.viewIdx === 0) {
      const enc = this.data.current ?? h[0] ?? null;
      return { enc, viewName: this.data.current ? 'Şu an' : enc ? 'Son savaş' : 'Şu an' };
    }
  ...
  render(force = false): void {
    ...
    if (!enc || enc.tallies.size === 0) {
      this.subEl.textContent = 'Henüz savaş kaydı yok.';
    ...
    this.subEl.textContent = isThreat
      ? (enc.mainMobName ? `Hedef: ${enc.mainMobName}` : 'Hedefle çatışma yok.')
