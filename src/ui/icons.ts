...
const PRIMITIVES = {
  ...
  cross(ctx, pal) {
    ctx.shadowColor = pal.glow; ctx.shadowBlur = 8;
    ctx.fillStyle = lin(ctx, -18, -22, 14, 22, [[0, '#ffffff'], [0.42, pal.light], [1, pal.base]]);
    ctx.beginPath();
    ctx.arc(2, 0, 22, Math.PI * 0.62, Math.PI * 1.38);
    ctx.arc(13, 0, 17, Math.PI * 1.38, Math.PI * 0.62, true);
    ctx.closePath();
    ctx.fill(); edge(ctx, pal.dark, 1.4);
    ctx.beginPath();
    for (let i = 0; i < 10; i++) {
      const a = -Math.PI / 2 + (i * Math.PI * 2) / 10;
      const r = i % 2 === 0 ? 6.2 : 2.7;
    ...
    ctx.closePath();
    ctx.fillStyle = pal.light;
    ctx.fill(); edge(ctx, pal.dark, 0.9);
    noShadow(ctx);
  },
  wing(ctx, pal) {
    ctx.translate(10, 8);
